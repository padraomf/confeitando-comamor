...
export async function infiniteCheckout(o:Order,s:Record<string,string>,origin:string){
 const addr=o.data.address;const r=await fetchJSON(infinite+'/links',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({handle:s.infiniteTag.replace(/^\$/,''),order_nsu:o.id,redirect_url:origin+'/pedidos',webhook_url:origin+'/api/webhooks/infinitepay',items:[...o.data.items.map(i=>({quantity:i.quantity,price:i.price,description:i.name})),...(o.data.fee?[{quantity:1,price:o.data.fee,description:'Entrega'}]:[])],...(addr?{address:{cep:(addr.cep||'').replace(/\D/g,''),number:addr.number||''}}:{}),customer:{name:o.data.profile.name,phone_number:'+55'+o.data.profile.phone,...(o.data.profile.email?{email:o.data.profile.email}:{}),...(o.data.profile.cpf?{document_number:o.data.profile.cpf.replace(/\D/g,'')}:{})}})});
