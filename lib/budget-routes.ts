...
export async function budgetRoute(req:Request,path:string,body:()=>Promise<any>){
 ...
 if(path==='admin/budgets'){
  ...
  if(method==='PATCH'){
   const input=z.object({id:z.string().uuid(),status:z.enum(['Recebido','Em análise','Encerrado']).optional(),offer:z.object({total:z.number().int().positive().max(10000000),fee:z.number().int().min(0).max(1000000),deposit:z.number().int().min(0).max(10000000),expires:dateTimeOrDate,terms:z.string().trim().min(5).max(2000)}).optional()}).parse(await body());
 ...
 if(path==='admin/budget'&&method==='POST'){
  ...
  const input=z.object({name:z.string().min(2),phone:z.string(),description:z.string().min(5),date:dateTimeOrDate,delivery:z.enum(['pickup','delivery']),address:addressSchema.optional(),offer:z.object({total:z.number().int().positive(),fee:z.number().int().min(0),deposit:z.number().int().min(0),expires:dateTimeOrDate,terms:z.string()})}).parse(await body());
