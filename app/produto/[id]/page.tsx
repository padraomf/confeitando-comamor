...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 ...
 return productMetadata(product,runtime.PUBLIC_URL||'https://confeitando-com-amor.ch4tgptpr0kotas.chatgpt.site',(await settings()).name);
}
export default async function ProductPage({params}:Props){
 const product=await loadProduct(productIdFromPath((await params).id));
 if(!product)notFound();
 const origin=runtime.PUBLIC_URL||'',store=await settings(),json={'@context':'https://schema.org','@type':'Product',name:product.name,description:product.description,image:origin+product.image,offers:{'@type':'Offer',price:(product.price/100).toFixed(2),priceCurrency:'BRL',availability:product.sold_out||product.stock===0?'https://schema.org/OutOfStock':'https://schema.org/InStock',seller:{'@type':'Organization',name:store.name}}};
